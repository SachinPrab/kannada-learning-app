import pronouns from '../data/kannada_pronouns.json';
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-r from-blue-100 via-white to-pink-100 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-2xl bg-white/80 rounded-xl shadow-lg p-6">
        <h1 className="text-3xl font-bold text-blue-700 mb-6 text-center">Pronouns List</h1>
        <ul className="space-y-4">
          {pronouns.map((pronoun) => (
            ...
            >
              <span className="font-semibold text-lg">{pronoun.english}</span>
              <span className="text-purple-900">{pronoun.kannada}</span>
              <span className="italic text-gray-600">{pronoun.transliteration}</span>
            </li>
          ))}
        </ul>
      </div>
